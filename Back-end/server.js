...
const maquiagens = require("./data/make.json");
...
//http://localhost:3000/api/maquiagens/aleatorio


app.get("/api/maquiagens/aleatorio", (req, res) => {
    //pegar todas as fotos de todas as raças
    // Object.values pega os valores do objeto
    // flat transforma tudo em um único array
    const todasAsFotos = Object.values(maquiagens).flat();
    ...
    res.json({
        ...
        message: `http://10.106.208.42:${PORT}/fotos/${item}`
    });
});
...
// ROTA 2 - maquiagem
//===================================
//http://localhost:3000/api/maquiagens/husky


app.get("/api/maquiagens/:tipos", (req, res) => {


    //pega o parâmetro da URL (ex. husky)
    const tipos = req.params.tipos.toLowerCase();
    ...
    if (!maquiagens[tipos]){
        res.status(404).json({
            ...
            message: `Maquiagem "${tipos}" não encontrada`
        });
        //encerra a execução da rota
        return;
    }
    //sortear uma foto da raça solicitada
    const item = sortear(maquiagens[tipos]);
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(`👻Servidor rodadando em http://0.0.0.0:${PORT}`);
    console.log(`📂Acesse de outra máquina usando: http://SEU_IP:${PORT}`);
